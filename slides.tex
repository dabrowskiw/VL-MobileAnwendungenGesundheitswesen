...
\begin{frame}{Rahmenbedingungen}
    ...
    \only<2->{
        Einige Beispiele:
        \begin{itemize}
            \item Fehldiagnose, zu später Arztbesuch
            \item<2-> ``29 Prozent der Deutschen sehr oder ziemlich an einem Service interessiert, der den möglichen Einfluss der aktuellen Ernährungsweise auf die künftige Gesundheit ermittelt'' \cite{InteresseKI} - zu hohes Vertrauen?
            \item<2-> Falsche Haltung/Intensität bei Übungen
            \item<3-> Abfluss persönlicher Daten: ``Gesundheits-Apps halten die datenschutzrechtlichen Anforderungen häufig nicht ein'' \cite{CHARISMABMG}
            \item<4-> Folgen von Fehlfunktionen, z.B. Einnahme falscher Medikamente
            \item<5-> Verlust von Körpergefühl, Verstärkung von Krankheitsängsten (Cyberchondrie als Weg zur Hypochondrie)
        \end{itemize}
    }
\end{frame}
...
\begin{frame}{Live-Minimalbeispiel revisited}
    ...
    \begin{itemize}
        ...
        \item Eigene Entscheidung - aber bitte gut überlegt!
    \end{itemize}
\end{frame}
